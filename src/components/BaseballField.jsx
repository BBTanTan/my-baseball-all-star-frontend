    ...
    <div className="relative w-full h-96 bg-gradient-to-b from-green-300 to-green-400 rounded-lg overflow-hidden">
      ...
      {positions.map(({ pos, x, y, label }) => {
        ...
        return (
          ...
          >
            <div className={`w-12 h-12 rounded-full border-2 flex items-center justify-center text-xs font-bold ${player ? 'bg-blue-500 border-blue-600 text-white' : isSelected ? 'bg-yellow-300 border-yellow-500 text-yellow-800' : 'bg-white border-gray-400 text-gray-600'} ${!readonly ? 'hover:shadow-lg' : ''}`}>
              {player ? (
