...
import { generatePlayers, Player, Position } from "@/utils/playerData";

interface TeamSelectionScreenProps {
  teamName: string;
  teamNumber: number;
  mode: 'random' | 'manual';
  onNext: (team?: Record<Position, Player | null>) => void;
  onBack: () => void;
}
...
const TeamSelectionScreen = ({
  ...
  const [selectedPlayers, setSelectedPlayers] = useState<Record<Position, Player | null>>({
    'C': null,
    'SP': null,
    'MR': null,
    'CL': null,
  ...
  useEffect(() => {
    if (mode === 'random') {
      const randomTeam: Record<Position, Player | null> = {} as Record<Position, Player | null>;
  ...
  const isPlayerAlreadySelected = (player: Player) => {
